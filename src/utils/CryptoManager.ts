...
export class CryptoManager {
  ...
  async initializeProductionKeys(userId: string): Promise<CryptoKey> {
    try {
      ...
      const keysResponse = await fetch(`/api/keys/public?exclude=${userId}`);
      if (!keysResponse.ok) {
      ...
      if (Object.keys(publicKeys).length === 0) {
        ...
        const groupResponse = await fetch('/api/keys/group', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json'
          },
        ...
        const groupResponse = await fetch(`/api/keys/group/global/${userId}`);
        if (!groupResponse.ok) {
