...
interface UserListProps {
  ...
  onStartPrivateChat?: (username: string) => void; // Nouvelle prop pour messages privés
}

export const UserList = ({ users, currentUser, isMobile = false, inChatInput = false, onStartPrivateChat }: UserListProps) => {
            ...
            <div className="max-h-48 overflow-y-auto scrollbar-thin scrollbar-thumb-red-700 scrollbar-track-black">
              {users.map(user => (
                ...
                >
                  ...
                  {user.username !== currentUser && onStartPrivateChat && (
                    <button
                      onClick={() => onStartPrivateChat(user.username)}
                      className="ml-auto text-red-400 hover:text-red-300 transition-colors p-1 hover:bg-red-700/20 rounded"
                      title="Message privé"
                    >
                      💬
                    </button>
        ...
        {users.map(user => (
          ...
          >
            ...
            {user.username !== currentUser && onStartPrivateChat && (
              <button
                onClick={() => onStartPrivateChat(user.username)}
                className="ml-auto text-red-400 hover:text-red-300 transition-colors p-2 hover:bg-red-700/20 rounded-lg border border-red-700/50 hover:border-red-500"
