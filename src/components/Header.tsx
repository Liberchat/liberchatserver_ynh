...
import { Palette, Accessibility, Sun, Moon, LogOut } from 'lucide-react';
  ...
  return (
    ...
    >
      <div className="flex items-center gap-1 sm:gap-4 min-w-0">
        ...
        <h1 className={`font-extrabold text-white uppercase tracking-widest truncate ${accessibilitySettings?.fontSize === 'large' ? 'hidden sm:block sm:text-3xl' :
      ...
      <div className="flex items-center gap-2 sm:gap-4">
        ...
        {customThemes && onApplyCustomTheme && (
          ...
          >
            <span className="inline-flex items-center">
              ...
              <span className={`ml-1 ${accessibilitySettings?.fontSize === 'large' || accessibilitySettings?.fontSize === 'xlarge'
                ? 'hidden' : 'hidden sm:inline'
                }`}>{t.themes.title.replace('🎨 ', '')}</span>
            </span>
          </button>
        )}
        ...
        {accessibilitySettings && onAccessibilityChange && (
          ...
          >
            <span className="inline-flex items-center">
              <Accessibility size={18} className="mr-1" />
              <span className={`ml-1 ${accessibilitySettings?.fontSize === 'large' || accessibilitySettings?.fontSize === 'xlarge'
                ? 'hidden' : 'hidden sm:inline'
                }`}>{t.accessibility.title}</span>
            </span>
          </button>
        )}
