    ...
    <>
      ...
      >
        ...
        <div className="flex items-center justify-between px-2 sm:px-4 py-2 sm:py-3">
          ...
          {/* Section droite : Boutons essentiels seulement */}
          <div className="flex items-center gap-0.5 sm:gap-2 flex-shrink-0">
            {/* Boutons essentiels - icônes seulement sur mobile */}
            ...
            >
              ...
              <span className="hidden sm:flex items-center gap-1 text-xs">⚙️ Admin</span>
            </button>

            {/* Connexion - si utilisateur connecté */}
            {currentUsername && (
              <button
            ...
            {/* Thème - masqué sur très petit écran */}
            {onToggleTheme && theme && (
              <button
                onClick={onToggleTheme}
                className={`p-1.5 sm:px-3 sm:py-1 rounded-full shadow border-2 border-red-700 font-bold transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-red-700 hidden xs:block ${theme === 'dark' ? 'bg-black/80 text-white hover:bg-white hover:text-red-700' : 'bg-white/90 text-black hover:bg-red-700 hover:text-white'}`}
            ...
            {/* Boutons secondaires - masqués sur mobile, accessibles via menu admin */}
            <div className="hidden lg:flex items-center gap-2 ml-2">
              {/* Thèmes personnalisables */}
              {customThemes && onApplyCustomTheme && (
                <button
                  onClick={() => setShowThemeCustomizer(true)}
                  className={`p-1.5 rounded-full shadow border-2 border-red-700 font-bold transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-red-700 ${theme === 'dark' ? 'bg-black/80 text-white hover:bg-white hover:text-red-700' : 'bg-white/90 text-black hover:bg-red-700 hover:text-white'}`}
              ...
              {onViewChange && (
                <div className="flex items-center gap-0.5 bg-gray-800 rounded-lg p-0.5 border border-red-800">
                  <button
                    ...
                    className={`px-2 sm:px-3 py-1 text-xs rounded font-medium transition-all duration-200 ${
                      currentView === 'chat' 
