...
import { EncryptionIndicator, type EncryptionStatus } from './EncryptionIndicator';
import { degradedModeManager } from '../utils/DegradedModeManager.ts';

// Fonction utilitaire pour s'assurer que le contenu est une chaîne
...
interface Message {
  ...
  encryptionStatus?: EncryptionStatus; // Statut de chiffrement du message
  decryptionError?: string; // Message d'erreur de déchiffrement
}
...
const ChatMessage: React.FC<ChatMessageProps> = ({ message, isOwnMessage, onDeleteMessage, onReply, socket, symmetricKey, encryptMessageE2EE, encryptMessageFallback, onRetryDecryption }) => {
  ...
  // Déterminer le statut de chiffrement du message
  const getEncryptionStatus = (): EncryptionStatus => {
    // Messages système ne sont pas chiffrés
    if (message.type === 'system') {
      return 'unsecured';
    }

    // Vérifier si c'est un message en mode dégradé
    if ((message as any).degradedMode === true) {
      return 'unsecured';
    }

    // Si le message a un statut explicite, l'utiliser
    if (message.encryptionStatus) {
      return message.encryptionStatus;
    }

    // Si il y a une erreur de déchiffrement
    if (message.decryptionError) {
      return 'error';
    }

    // Si on a une clé symétrique et que le contenu semble chiffré
    if (symmetricKey) {
      // Vérifier si le contenu ressemble à du JSON chiffré
      if (message.type === 'text' && message.content) {
        const content = getMessageContent(message);
        if (content) {
          const isEncryptedContent = content.trim().startsWith('{') && 
                                   content.trim().endsWith('}') &&
                                   /[{}\[\]"]/.test(content);
          if (isEncryptedContent) {
            return 'error'; // Contenu chiffré non déchiffré
          }
        }
      }
      return 'encrypted';
    ...
    // Pas de clé = non sécurisé
    return 'unsecured';
  };

  const handleRetryDecryption = () => {
    if (onRetryDecryption) {
      onRetryDecryption(message.id);
    }
  };
  ...
  const LinkPreview: React.FC<{ url: string }> = ({ url }) => {
    ...
    React.useEffect(() => {
      ...
      const apiUrl = basePath ? `${basePath}/api/link-preview?url=${encodeURIComponent(url)}` : `/api/link-preview?url=${encodeURIComponent(url)}`;
      fetch(apiUrl)
        .then(r => r.json())
  ...
  const renderContent = () => {
    if (message.type === 'text') {
      // Vérifier si le contenu est du JSON chiffré non déchiffré
      const content = getMessageContent(message);
      if (!content || content.trim() === '') {
        return null;
      }

      // Si le contenu ressemble à du JSON chiffré, afficher un message d'erreur
      const isEncryptedContent = content.trim().startsWith('{') && 
                               content.trim().endsWith('}') &&
                               /[{}\[\]"]/.test(content);
        ...
        {message.username && message.type !== 'system' && (
          <span className="text-xs sm:text-sm font-bold uppercase tracking-wider text-red-300 mb-1 block font-mono flex items-center gap-2">
            ...
            <EncryptionIndicator 
              ...
              size="sm"
              onClick={getEncryptionStatus() === 'error' ? handleRetryDecryption : undefined}
              errorMessage={message.decryptionError}
            />
          </span>
        )}
