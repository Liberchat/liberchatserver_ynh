...
interface ChatMessageProps {
  ...
  encryptMessageFallback?: (message: string, keyHex: string) => { iv: number[]; content: string };
  encryptMessageWasm?: (text: string) => Promise<Uint8Array>;
  toBase64Wasm?: (data: Uint8Array) => string;
  isWasmReady?: boolean;
...
const ChatMessage: React.FC<ChatMessageProps> = ({
  ...
  const renderContent = () => {
    if (message.type === 'text') {
      ...
      // Détection JSON/Chiffré non déchiffré
      if (contentStr.trim().startsWith('{') && contentStr.trim().endsWith('}')) {
        return (
          <div className="text-xs text-red-400 font-mono p-2 border border-red-900/50 rounded bg-black/40 break-all select-all">
            🔒 Message modifé chiffré (Déchiffrement impossible)
            {/* Debug: {contentStr.slice(0, 20)}... */}
          </div>
        );
      }
        ...
        return (
          <div className="max-w-[160px] sm:max-w-[220px] flex flex-col items-center">
            <img
              ...
              onClick={(e) => handleImageEvent(e)}
              onContextMenu={(e) => handleImageEvent(e)}
              onTouchStart={(e) => handleImageEvent(e)}
              role="button"
              tabIndex={0}
  ...
  const handleImageEvent = (e: React.MouseEvent | React.TouchEvent) => {
    e.stopPropagation();
  ...
  const handleEditSubmit = async (e: React.FormEvent) => {
    ...
    if (message.type === 'text') {
      if (editValue.trim() && editValue !== message.content && socket) {
        ...
        try {
          if (isWasmReady && encryptMessageWasm && toBase64Wasm) {
            // Option 1 : Libsodium (WASM) - Prioritaire
            const raw = await encryptMessageWasm(editValue);
            encryptedContent = toBase64Wasm(raw);
            socket.emit('edit message', { id: message.id, content: encryptedContent });
          } else if (window.crypto && window.crypto.subtle && typeof symmetricKey !== 'string' && symmetricKey) {
            // Option 2 : WebCrypto (Legacy)
            const payload = await encryptMessageE2EE(editValue, symmetricKey);
            encryptedContent = JSON.stringify(payload);
            socket.emit('edit message', { id: message.id, content: encryptedContent });
          } else if (typeof symmetricKey === 'string' && encryptMessageFallback) {
            // Option 3 : Fallback JS
            const payload = encryptMessageFallback(editValue, symmetricKey);
            encryptedContent = JSON.stringify(payload);
            socket.emit('edit message', { id: message.id, content: encryptedContent });
          } else {
            alert("Aucune méthode de chiffrement disponible pour l'édition");
            return;
          }
          setIsEditing(false);
        } catch (error) {
          console.error("Erreur chiffrement edit:", error);
    ...
    // Si c'est un fichier ou audio
    else if ((message.type === 'file' || message.type === 'audio') && editFile && socket) {
      const reader = new FileReader();
      reader.onload = async () => {
        const fileData = reader.result as string;
        let encryptedFile;
        // Pour les fichiers, on garde WebCrypto pour l'instant (WASM chunking TODO)
        if (window.crypto && window.crypto.subtle && typeof symmetricKey !== 'string' && symmetricKey && encryptMessageE2EE) {
          encryptedFile = await encryptMessageE2EE(fileData, symmetricKey);
        } else if (typeof symmetricKey === 'string' && encryptMessageFallback) {
          encryptedFile = encryptMessageFallback(fileData, symmetricKey);
        } else {
          alert('Chiffrement fichier indisponible');
          return;
        }
  ...
  const iframeTimeoutRef = React.useRef<ReturnType<typeof setTimeout> | null>(null);
  React.useEffect(() => {
    if (!iframeUrl) return;
    setIframeError(false);
  ...
  const handleReact = async (emoji: string) => {
    // Si une fonction onReact est fournie (ex: chat privé), on l'utilise en priorité
    if (onReact) {
    ...
    try {
      if (isWasmReady && encryptMessageWasm && toBase64Wasm) {
        // Libsodium
        const payload = JSON.stringify({ emoji, username: message.username }); // Pourquoi crypter username ?
        // On chiffre juste l'emoji ? Non, le payload complet pour context.
        const raw = await encryptMessageWasm(payload);
        const encrypted = toBase64Wasm(raw);
        socket.emit('react message', { messageId: message.id, encrypted });

      } else if (symmetricKey && encryptMessageE2EE) {
        // WebCrypto
      ...
      console.error("Erreur chiffrement réaction", e);
    }
  ...
  const handleTranslation = (translated: string, language: string) => {
      ...
      <div className="flex gap-1 mt-1 flex-wrap">
        {Object.entries(message.reactions).map(([emoji, users]) => {
          ...
          return (
            ...
            >
              {emoji} <span className="text-xs text-gray-300">{userList.length}</span>
            </button>
          );
        })}
      </div>
  ...
  return (
    ...
    >
      ...
      >
        ...
        {message.type === 'text' && message.content && !isOwnMessage && (
          <AutoTranslation
            text={message.content}
            targetLanguage={autoTranslationLanguage}
            enabled={autoTranslationEnabled}
          />
        )}
