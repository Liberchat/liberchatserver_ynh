...
function App() {
  ...
  useEffect(() => {
    ...
    newSocket.on('connect_error', (err: any) => {
      ...
      // En développement, essaie une connexion directe si le proxy échoue
      if (import.meta.env.DEV && socketUrl.includes('5173')) {
        console.log('Tentative de connexion directe au serveur backend...');
        setTimeout(() => {
          ...
          setSocket(directSocket);

          directSocket.on('connect', () => {
            console.log('Connexion directe réussie');
            setIsConnected(true);
          });
          directSocket.on('disconnect', () => setIsConnected(false));
        }, 2000);
      }
    });
