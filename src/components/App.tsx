...
function AppContent() {
  ...
  async function encryptMessageE2EE(message: string, key: CryptoKey | string) {
    if (key === 'wasm-initialized' && useWasm && wasmReady) {
      const encrypted = await encryptWasm(message);
      return {
        wasm: true,
  ...
  async function decryptMessageE2EE(encrypted: any, key: CryptoKey | string) {
    if (key === 'wasm-initialized' && useWasm && wasmReady && encrypted && encrypted.wasm) {
      try {
        const encryptedBytes = base64ToUint8Array(encrypted.content);
        return await decryptWasm(encryptedBytes);
      } catch (e) {
        console.error('Erreur déchiffrement WASM dans decryptMessageE2EE:', e);
        return encrypted.content;
      }
    }
    if (window.crypto && window.crypto.subtle && typeof key !== 'string') {
