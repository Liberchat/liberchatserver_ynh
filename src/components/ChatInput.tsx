...
interface ChatInputProps {
  ...
  isConnected: boolean;
  users?: Array<{ username: string; socketId: string }>; // Optionnel maintenant
  currentUser: string;
  ...
  onToggleTranslationSettings?: () => void;
  onTyping?: () => void;
  onStopTyping?: () => void;
  disabled?: boolean;
  placeholder?: string;
    ...
    <form onSubmit={handleSubmit} className="sticky bottom-0 left-0 w-full max-w-full min-h-[56px] flex flex-wrap items-end gap-2 bg-black border-t-4 border-red-700 p-2 sm:p-4 relative z-50">
      ...
      <div className="flex gap-2 w-full sm:w-auto order-1 sm:order-none">
        ...
        <button
          ...
          className={`flex-shrink-0 w-10 h-10 p-0 rounded-lg border-2 border-white transition-colors flex items-center justify-center ${isRecording ? 'bg-red-700 text-white' : 'bg-black text-red-700 hover:bg-red-700/20'}`}
        ...
        {onToggleTranslationSettings && (
          <button
            ...
            className="flex-shrink-0 w-10 h-10 p-0 rounded-lg border-2 border-white bg-black text-gray-400 hover:text-white hover:bg-red-700/20 transition-colors flex items-center justify-center"
