...
import { Smile, Paperclip, Mic, Square, X } from 'lucide-react';
    ...
    <form onSubmit={handleSubmit} className="sticky bottom-0 left-0 w-full max-w-full min-h-[56px] flex flex-wrap items-end gap-2 bg-black border-t-4 border-red-700 p-2 sm:p-4 relative z-50">
      ...
      {replyTo && (
        <div className="absolute -top-20 left-0 w-full flex items-center justify-between px-3 py-2 bg-black/90 border-l-4 border-red-700 rounded-t-lg rounded-b-sm shadow-lg" style={{ minHeight: '48px' }}>
          ...
          <button type="button" onClick={() => onReplyHandled && onReplyHandled()} className="ml-2 text-gray-400 hover:text-red-500 p-0.5 rounded focus:outline-none" title={t.chat.cancel} aria-label={t.chat.cancel}>
            <X size={20} />
          </button>
        </div>
      )}
      ...
      <div className="flex gap-2 w-full sm:w-auto order-1 sm:order-none">
        <button
          ...
          className="flex-shrink-0 p-2 h-10 bg-black border-2 border-white rounded-lg hover:bg-red-700/20 transition-colors flex items-center justify-center"
        >
          <Smile size={20} className="text-white" />
        </button>
        <label className="flex-shrink-0 cursor-pointer p-2 h-10 bg-red-700 text-white rounded-lg border-2 border-white hover:bg-black hover:text-red-700 transition-colors flex items-center justify-center">
          <Paperclip size={18} />
          <input
            type="file"
            accept="image/*"
      ...
      <div className="flex-1 relative order-3 sm:order-none">
        ...
        {showMentions && filteredUsers.length > 0 && (
          <div className="absolute bottom-full left-0 mb-2 z-50 bg-black border-2 border-red-700 rounded-lg shadow-lg w-64 max-w-[90vw] max-h-48 overflow-y-auto
                          sm:left-0
