...
app.use(cors({
  origin: (origin, callback) => {
    ...
    if (origin && origin.startsWith('https://')) return callback(null, true);

    // Autorise localhost sur n'importe quel port
    if (origin && origin.match(/^https?:\/\/localhost(:\d+)?$/)) return callback(null, true);

    // Autorise tous les domaines .noho.st (YunoHost)
    if (origin && origin.includes('.noho.st')) return callback(null, true);

    // Log pour debug
    console.log('CORS origin refusé:', origin);
    callback(null, true); // Temporairement autorise tout pour debug
  },
...
// Configuration Socket.IO avec debug
// CORRECTION: Utiliser le basePath pour que Socket.IO corresponde au proxy nginx
...
// CORRECTION CRITIQUE: Initialiser Socket.IO APRÈS avoir configuré Express
...
console.log('✅ Socket.IO server créé avec le chemin:', socketPath);

app.use(express.json({ limit: '50mb' }));
app.use(express.urlencoded({ limit: '50mb', extended: true }));

// Middleware de debug simple (ne pas interférer avec Socket.IO)
...
if (basePath) {
  ...
  app.use(basePath, express.static(publicPath)); // Servir aussi les fichiers public
...
// Route pour récupérer les clés publiques
app.get('/api/keys/public', (req, res) => {
  // Version simplifiée - retourne un objet vide (pas de clés stockées)
  res.json({ publicKeys: {} });
});

// Route pour enregistrer une clé de groupe
app.post('/api/keys/group', express.json(), (req, res) => {
...
// Route pour récupérer une clé de groupe
app.get('/api/keys/group/:groupId/:userId', (req, res) => {
  const { groupId, userId } = req.params;

  // Version simplifiée - retourne une clé factice
  res.json({
    success: true,
    groupKeyData: null,
    message: 'Pas de clé de groupe stockée'
  });
});
...
// Route de test pour vérifier Socket.IO directement
app.get('/test-socketio', (req, res) => {
  const fullSocketPath = basePath ? `${basePath}/socket.io/` : '/socket.io/';
  res.send(`
    <!DOCTYPE html>
    <html>
    <head><title>Test Socket.IO</title></head>
    <body>
      <h1>Test Socket.IO</h1>
      <div id="status">Connexion...</div>
      <script src="${fullSocketPath}socket.io.js"></script>
      <script>
        ...
        socket.on('connect', () => {
          document.getElementById('status').innerHTML = '✅ Connecté! ID: ' + socket.id;
        });
...
// Route pour servir le test réseau
app.get('/test-network', (req, res) => {
  const baseUrl = req.protocol + '://' + req.get('host') + (basePath || '');
  const fullSocketPath = basePath ? `${basePath}/socket.io/` : '/socket.io/';
  const socketUrl = req.protocol + '://' + req.get('host') + fullSocketPath;
  
  const html = '<!DOCTYPE html><html><head><title>Test Réseau LiberChat</title></head><body>' +
    '<h1>Test de Connectivité LiberChat</h1><div id="results"></div>' +
    '<script>' +
    ...
    '}' +
    'log("🔍 Test 1: Connectivité de base...");' +
    'fetch("' + baseUrl + '/").then(r => r.ok ? log("✅ Serveur accessible", "success") : log("❌ Erreur " + r.status, "error")).catch(e => log("❌ " + e.message, "error"));' +
    'log("🔍 Test 2: Socket.IO...");' +
    'fetch("' + socketUrl + '?EIO=4&transport=polling").then(r => r.ok ? log("✅ Socket.IO OK", "success") : log("❌ Socket.IO " + r.status, "error")).catch(e => log("❌ " + e.message, "error"));' +
    '</script></body></html>';
...
app.get('/api/stats', (req, res) => {
  const stats = {
    ...
    lastBackup: new Date().toISOString(),
    socketIOPath: socketPath,
    fullSocketIOPath: basePath ? `${basePath}/socket.io/` : '/socket.io/'
  };
  res.json(stats);
});
...
app.use((req, res, next) => {
  ...
  if (req.url.includes('socket.io')) {
    console.log('🔌 Requête Socket.IO détectée:', {
      method: req.method,
      url: req.url,
      path: req.path,
...
io.on('connection', (socket) => {
  ...
  console.log('Origine:', socket.handshake.headers.origin);
  ...
  socket.on('disconnect', (reason) => {
    ...
    if (user) {
      ...
      io.emit('users', Array.from(users.values()));
      console.log('❌ Utilisateur déconnecté:', encodeURIComponent(user.username), 'Raison:', reason);
    } else {
      console.log('❌ Déconnexion socket sans utilisateur:', socket.id, 'Raison:', reason);
    }
  });
